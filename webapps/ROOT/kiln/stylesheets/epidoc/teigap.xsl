   ...
   <xsl:template name="extent-string">
      ...
      <xsl:choose>
         <xsl:when test="@extent='unknown'">
            <xsl:choose>
               ...
               <xsl:when test="$parm-leiden-style = 'panciera'">
                  ...
                  <xsl:if test="@cert='low'"><xsl:text> ?</xsl:text></xsl:if> <!-- added for PPRET -->
               </xsl:when>
               <!--<xsl:otherwise>
                  <xsl:text>-\-\-</xsl:text>
               </xsl:otherwise>-->
            ...
            <xsl:choose>
               ...
               <xsl:when test="number(@quantity) &gt; $cur-max or (number(@quantity) &gt; 1 and @precision='low')">
                  <xsl:choose>
                     ...
                     <xsl:when test="$parm-leiden-style = 'panciera'"> <!-- changed for PPRET -->
                        <xsl:choose>
                           <xsl:when test="@precision='low' or @cert='low'">
                              <xsl:text>- </xsl:text>
                              <xsl:if test="@precision='low'"><xsl:text>ca. </xsl:text></xsl:if>
                              <xsl:value-of select="@quantity"/>
         ...
         <xsl:when test="@quantity and @unit='line'">
            <xsl:choose>
               <xsl:when test="$parm-leiden-style = 'panciera'"> <!-- added for PPRET -->
                  <xsl:text>- - - </xsl:text>
                  <xsl:if test="@precision='low'">ca. </xsl:if>
                     <xsl:value-of select="@quantity"/>
                  <xsl:text> line</xsl:text><xsl:if test="number(@quantity) &gt; 1"><xsl:text>s</xsl:text></xsl:if>
               ...
               <xsl:when test="$parm-leiden-style = 'eagletxt' and not(following-sibling::t:lb)"> <!-- removed panciera for PPRET -->
                  <xsl:text>------</xsl:text>
               </xsl:when>
               <xsl:when test="$parm-leiden-style = 'edh-itx'">
                  <xsl:choose>
