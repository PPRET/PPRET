...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="t:space">
      ...
      <!-- function EDF:f-wwrap declared in htm-teilb.xsl; tests if lb break=no immediately follows space -->  <!-- changed for PPRET -->   
                ...
                <xsl:when test="$parm-leiden-style='panciera'">
                  ...
                  <xsl:choose>
                     <xsl:when test="@extent = 'unknown'">
                        <!-- Found in [htm|txt]-teispace.xsl -->
                        <xsl:call-template name="space-content">
                           <xsl:with-param name="vacat" select="'vac.'"/> <!-- modified for PPRET -->
                        </xsl:call-template>
                     </xsl:when>
                     <xsl:when test="@quantity and @unit='character'">
                        <xsl:call-template name="space-content">
                     ...
                     <xsl:when test="@quantity and @unit='line'">
                        <xsl:text>vacat</xsl:text> <!-- changed for PPRET -->
                        <!--<xsl:call-template name="space-content">
                        <!-\- Found in [htm|txt]-teispace.xsl -\->
                           <xsl:with-param name="vacat"><xsl:text> vacat </xsl:text></xsl:with-param>
                           <xsl:with-param name="extent">
                              <xsl:value-of select="@quantity"/>
                              <xsl:text> line</xsl:text>
