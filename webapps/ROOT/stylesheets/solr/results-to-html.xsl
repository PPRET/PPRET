...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:template match="lst[@name='facet_fields']/lst" mode="search-results">
    ...
    <section>
      ...
      <div class="content" data-section-content="">
        ...
        <ul class="no-bullet"><xsl:apply-templates mode="search-results" /></ul>
      </div>
    </section>
  </xsl:template>
  ...
  <xsl:template match="result/doc" mode="search-results">
    ...
    <xsl:variable name="result-url">
      <xsl:choose>
        ...
        <xsl:when test="$document-type = 'epidoc'">
          <xsl:choose>
            <xsl:when test="$q_value"><xsl:value-of select="concat(kiln:url-for-match('local-epidoc-display-html', ($language, $short-filepath), 0), '?q=', $q_value)" /></xsl:when>
            <xsl:otherwise><xsl:value-of select="kiln:url-for-match('local-epidoc-display-html', ($language, $short-filepath), 0)" /></xsl:otherwise>
          </xsl:choose>
        </xsl:when>
      </xsl:choose>
    </xsl:variable>
    <li>
        <a href="{$result-url}">
        <xsl:value-of select="concat(str[@name='document_id'], '. ', arr[@name='document_title']/str)"/>
      </a>
    </li>
  </xsl:template>
