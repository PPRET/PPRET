      ...
      <xsl:for-each-group select="//tei:placeName[ancestor::tei:div/@type='edition']" group-by=".">
        <doc>
          ...
          <field name="index_item_name">
            ...
            <xsl:variable name="place-id" select="document('../../content/xml/authority/listPlace.xml')//tei:place[@xml:id=$pl-id]"/>
            <xsl:choose>
              <xsl:when test="@ref ">
                <xsl:choose>
                  <xsl:when test="$place-id">
                      <xsl:value-of select="$place-id/tei:placeName[@xml:lang='en']"/>
                  </xsl:when>
